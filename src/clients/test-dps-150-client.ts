...
export type TestDPS150Callback = (data: any) => void;

export class TestDPS150Client implements DeviceClient {
  private mockPort: SerialPort;
  private callback: TestDPS150Callback;
  ...
  constructor(port: SerialPort, callback: TestDPS150Callback) {
    this.mockPort = port; // This will be a mock port
    this.callback = callback;
  }

  async start(): Promise<void> {
    console.log('TestDPS150Client start - simulating serial connection');
    // In test mode, we don't actually open the mock port, just start sending data
    this.prePopulateHistoricalData();
    this.startSendingUpdates();
  }

  async stop(): Promise<void> {
    console.log('TestDPS150Client stop');
    if (this.intervalId) {
  ...
  private prePopulateHistoricalData() {
    ...
    for (let i = historicalPoints; i > 0; i--) {
      ...
      // Send historical data point - use same format as real client
      this.callback(historicalData);
    }
  }
